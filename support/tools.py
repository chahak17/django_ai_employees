...
def check_delivery_status(tracking_number, carrier):
    default_response= {
                 "status": "Unknown",
                "last_location": "tracking info unavialable",
                "last_update": "N/A",
...
def get_customer_risk_profile(user_id):
    ...
    if total_orders > 0 :
        refund_to_order_ratio = round(total_refunds / total_orders, 2) # order = 8 , refund = 3 (safe) o=1
    else:
